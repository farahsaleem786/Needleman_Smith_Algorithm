...
def main():
    main_window = Tk()
    app = smith_watermanGUI(main_window)
    main_window.mainloop()
class smith_watermanGUI:
    def __init__(self,root):

        self.root = root
        self.root.configure(bg="white")
        self.root.title("SMITH WATER MAN ALGORITHM")
        def on_closing():
        ...
        START = tk.Label(frame1, text="SMITH WATER MAN ALGORITHM", font=("bold", 25), bg="light blue", fg="white", height=2)
        ...
        # btn_GRAPH = tk.Button(my_canvas)
        #     , text='SHOW GRAPH', command=self.create_alignment_pie_chart,
        #                      font=("Times New Roman", 14, 'bold'), bd=3, relief= tk.RIDGE,
        #                      cursor='hand2', bg='ORANGE', fg='white', activeforeground='white', activebackground='#333637')
        # # btn_back.grid(row=0, column=3,sticky="ne", columnspan=2)
        # btn_GRAPH.place(x=screen_width-190, y=screen_height-710, width=170)
        # self.canvas=tk.Canvas(my_canvas,bg="WHITE")
        #  Figure and Axes
        # Create a container frame for the graphAa
        # fig_size = (22, 20)
        # self.fig, self.ax = plt.subplots(figsize=fig_size)
        # self.ax.axis('equal')  # Equal aspect ratio ensures circular shape
        # self.ax.set_title('Sequence Alignment')
        ...
        # Configure content_frame to expand its rows and columns
    ...
    def align_sequences(self):
        ...
        self.alignment_text.grid(row=10, column=1, columnspan=2, sticky="nsew", padx=10, pady=10)
        # self.canvas_widget.place(x=screen_width-600, y=screen_height-750, width=480,height=460)
        ...
        try:
            ...
            alignment, alignment_score, main_matrix = self.smith_waterman(seq1, seq2,match_reward,mismatch_penalty,gap_penalty)
    ...
    def smith_waterman(self, seq1, seq2, match_reward, mismatch_penalty, gap_penalty):
            # Create matrices and perform the Smith-Waterman algorithm
            ...
            for i in range(1, len(seq1) + 1):
                for j in range(1, len(seq2) + 1):
                    main_matrix[i][j] = max(
                        ...
                        0  # Additional check for Smith-Waterman: If the calculated value is negative, set it to 0
                    )
            ...
            # traceback
            while ti > 0 and tj > 0 and main_matrix[ti][tj] != 0:
                if main_matrix[ti][tj] == main_matrix[ti - 1][tj - 1] + match_checker_matrix[ti - 1][tj - 1]:
                    ...
                    ti -= 1
                    tj -= 1
                elif main_matrix[ti][tj] == main_matrix[ti - 1][tj] + gap_penalty:
            ...
            # btn_GRAPH = tk.Button(text='SHOW GRAPH', command=self.create_alignment_pie_chart(aligned_1, aligned_2),
            # font=("Times New Roman", 14, 'bold'), bd=3, relief= tk.RIDGE,
            # cursor='hand2', bg='ORANGE', fg='white', activeforeground='white', activebackground='#333637')
            # # btn_back.grid(row=0, column=3,sticky="ne", columnspan=2)
            # screen_width = self.root.winfo_screenwidth()
            ...
            print(aligned_1)
            print(len(aligned_2))
            return (aligned_1, aligned_2), max_score, main_matrix
            ...
            for j in range(len(seq2) + 2):
                ...
                if i == 0 and j == 0:
                    ...
                    if (i, j) != (1, 0) and (i, j) != (0, 1):  # Add condition to skip (1, 0) and (0, 1)
                        # Check if the cell contains an arrow and update its color
